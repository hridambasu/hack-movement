...
impl Blockchain{
    // Helper method to calculate the height of a chain recursively
    fn calculate_chain_height(&self, block_hash: &str, visited: &HashSet<String>) -> u64 {
        if visited.contains(block_hash) {
            return 0;
        }

        let (_, parents) = self.blocks.get(block_hash).unwrap();
        let mut height = 1;

        for parent_hash in parents {
            height += self.calculate_chain_height(parent_hash, visited);
        }

        height
    }

    // Helper method to construct the chain recursively
    fn construct_chain_recursive(&self, block_hash: &str, chain: &mut Vec<Block>, visited: &mut HashSet<String>) {
        if visited.contains(block_hash) {
            return;
        }
        ...
        let (block, parents) = self.blocks.get(block_hash).unwrap();
        for parent_hash in parents {
            self.construct_chain_recursive(parent_hash, chain, visited);
        }
...
impl BlockchainOperations for Blockchain {
    ...
    /*async fn get_main_chain(&self) -> Result<Vec<Block>, String> {
        // Implement the Longest Chain Rule here
        let mut main_chain = Vec::new();
        let mut visited = HashSet::new();

        // Find the block with the maximum height (cumulative difficulty/length)
        let mut max_height = 0;
        let mut max_height_block = None;

        for (block_hash, (_, parents)) in &self.blocks {
            if !visited.contains(block_hash) {
                let height = self.calculate_chain_height(block_hash, &visited);
                if height > max_height {
                    max_height = height;
                    max_height_block = Some(block_hash.clone());
                }
            }
        }

        // Reconstruct the main chain starting from the block with maximum height
        if let Some(start_block) = max_height_block {
            self.construct_chain_recursive(&start_block, &mut main_chain, &mut visited);
        }
